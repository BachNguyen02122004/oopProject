...
public class DictionaryManagement {
    ...
    public void sortDictionary() {
        Collections.sort(dictionaryWords, new Comparator<Word>() {
            ...
            public int compare(Word word1, Word word2) {
                ...
                return word1.getWord_targer().compareTo(word2.getWord_targer());
            }
        });
    }
    ...
    public String dictionaryLookup() {
        ...
        while (l <= r) {
            int m = (l + r) / 2;
            if (dictionaryWords.get(m).getWord_targer().equals(word_target)) {
                String ans = dictionaryWords.get(m).getWord_explain();
                return ans;
            } else if (dictionaryWords.get(m).getWord_targer().compareTo(word_target) < 0) {
                l = m + 1;
            } else {
                r = m - 1;
            }
